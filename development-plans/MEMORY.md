...
**Commit attribution update (2026-09-17):** At the owner's explicit request, future automated state commits use author `asteroidcrib729 <132701966+asteroidcrib729@users.noreply.github.com>` while retaining `Job Finder Bot <actions@github.com>` as committer. This restores the account-linked authorship used before the September 8 state-saving refactor, regardless of who triggers the workflow. Existing commit history is not rewritten. The local bare-repository reconciliation regression verifies both identities on the actual pushed commit after a concurrent-branch retry.
